...
function setSalesAmount(period, reportType, labelTotalItems) {
	...
	$("#textTotalNetSales" + reportType).text(formatCurrency(totalNetSales) + " = SubTotal - ProductCost ");
	
	denominator = getDenominator(period, reportType);
	if(denominator == 6 || denominator == 12){
		$("#textAvgGrossSales" + reportType).text(formatCurrency(totalGrossSales / denominator) + " = TotalGrossSales/month");
		$("#textAvgNetSales" + reportType).text(formatCurrency(totalNetSales / denominator) + " = TotalNetSales/month");

	}
	else {
		$("#textAvgGrossSales" + reportType).text(formatCurrency(totalGrossSales / denominator) + " = TotalGrossSales/day");
		$("#textAvgNetSales" + reportType).text(formatCurrency(totalNetSales / denominator) + " = TotalNetSales/day");
